...
def create_chunk_generator(book, chunk_size):
    ...
    filtered_lines = []
    copyright_words = [
    'rights', 'reserved', 'reproduced', 'copyright', 'reproduce', 'permission',

    # 中文关键词
    '版权所有', '著作权', '未经允许', '不得转载', '未经授权', '保留所有权利',
    '版权声明', '侵权必究', '仅供参考', '仅供学习', '如需转载请', '转载注明',
    '电子书', '本作品受', '版权归', '仅限内部', '仅限交流', '下载后请在',
    '请在下载后', '请勿用于商业', '不得用于商业', '请支持正版', '支持正版',
    '请购买正版', '本书来自网络', '整理自网络', '由网络收集'
    ]
...
5. Output in the same language as the input. (In the output the key should always in English but the value depends on the language of the input)
    ...
    def parse_response(response, chunk, book, **kwargs):
        ...
        try:
            ...
            for unprocessed_plot in response['plots']:
                ...
                if 'chapter_title' in unprocessed_plot:
                    chapter_title = unprocessed_plot['chapter_title']
                else:
                    chapter_title = unprocessed_plot['chapter']
                    unprocessed_plot['chapter_title'] = unprocessed_plot['chapter']
...
4. Output in the same language as the input. (In the output the key should always in English but the value depends on the language of the input)
...
"""
    ...
    prompt = """Given a list of character names, titles, or form of address, your task is to: i) generate a list of named characters with their official names (in {language}); ii) For each name in the given list, align it with the official character name if it refers to a named character, or denote it as "impersonal" otherwise. iii)Output in the same language as the input. (In the output the key should always in English but the value depends on the language of the input)
